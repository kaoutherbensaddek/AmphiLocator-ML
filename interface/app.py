...
======================================================
Run:   streamlit run app.py
Deps:  pip install streamlit pandas numpy plotly scikit-learn joblib
"""

import math, hashlib, time, socket, warnings
from datetime import datetime
...
# ═══════════════════════════════════════════════════════════════════════════════
#  ██████  MODEL LOADING  ██████
#  All model-related code lives in this block.
#  To add a new model later:
#    1. Add joblib.load(...) here under load_models()
#    2. Add its key to the returned dict
#    3. Use it in build_features() or classify()
# ═══════════════════════════════════════════════════════════════════════════════
import joblib

MODELS_DIR = Path(__file__).parent.parent / "models"   # ../models relative to app.py

@st.cache_resource(show_spinner="Loading models…")
def load_models():
    """
    Loads all .pkl models once at startup and caches them.
    Returns a dict so you can access any model by name.

    Keys:
      'gbm'          → GradientBoosting (primary / best model)
      'knn'          → KNeighborsClassifier
      'lr'           → LogisticRegression
      'outside'      → outside_detector binary classifier (runs first as a gate)
      'label_enc'    → LabelEncoder  (converts numeric prediction → "Amphi X")
    """
    models = {}
    model_files = {
        "gbm":      "gbm_best.pkl",
    ...
    for key, filename in model_files.items():
        ...
        if path.exists():
            ...
            models[key] = None   # graceful fallback — won't crash the app
    return models

MODELS = load_models()

# ── Which model to use for the main amphi prediction ──────────────────────────
# Change this one line to switch between 'gbm', 'knn', or 'lr'
ACTIVE_MODEL_KEY = "gbm"


def build_features(lat: float, lon: float, accuracy: float) -> pd.DataFrame:
    """
    ── ADD NEW FEATURES HERE ──
    Converts raw GPS input into the exact feature vector the models were trained on.
    Must match the SCALED_FEATURES + BINARY_FEATURES order from 04_KNN_LogReg.ipynb.

    If you add features in a future notebook, add them here too.
    """
    # Centroid distances (metres via haversine)
    dists = {n: haversine(lat, lon, c[0], c[1]) for n, c in CENTROIDS.items()}
    dist_vals = list(dists.values())
    sorted_dists = sorted(dist_vals)
    nearest_d  = sorted_dists[0]
    second_d   = sorted_dists[1]

    # Nearest amphi index (0–7) — one-hot encoded below
    nearest_name = min(dists, key=dists.get)
    nearest_idx  = list(CENTROIDS.keys()).index(nearest_name)

    # Scaling factors (reproduce the same scaling used during training)
    # These should match your scaler from 03_feature_engineering.ipynb
    # If you saved a scaler.pkl, load it here and use scaler.transform() instead
    ACC_MEAN, ACC_STD   = 22.0,  15.0    # ← replace with your actual train mean/std
    DIST_MEAN, DIST_STD = 50.0,  40.0
    LOG_ACC_MEAN        = np.log1p(22.0)
    LOG_ACC_STD         = 0.8

    scaled = {
        "accuracy_mean_scaled": (accuracy - ACC_MEAN) / ACC_STD,
        "dist_Amphi_1_scaled":  (dists["Amphi 1"] - DIST_MEAN) / DIST_STD,
        "dist_Amphi_2_scaled":  (dists["Amphi 2"] - DIST_MEAN) / DIST_STD,
        "dist_Amphi_3_scaled":  (dists["Amphi 3"] - DIST_MEAN) / DIST_STD,
        "dist_Amphi_4_scaled":  (dists["Amphi 4"] - DIST_MEAN) / DIST_STD,
        "dist_Amphi_5_scaled":  (dists["Amphi 5"] - DIST_MEAN) / DIST_STD,
        "dist_Amphi_6_scaled":  (dists["Amphi 6"] - DIST_MEAN) / DIST_STD,
        "dist_Amphi_7_scaled":  (dists["Amphi 7"] - DIST_MEAN) / DIST_STD,
        "dist_Amphi_8_scaled":  (dists["Amphi 8"] - DIST_MEAN) / DIST_STD,
        "dist_nearest_scaled":  (nearest_d - DIST_MEAN) / DIST_STD,
        "dist_2nd_scaled":      (second_d  - DIST_MEAN) / DIST_STD,
        "dist_gap_scaled":      ((second_d - nearest_d) - 0.0) / DIST_STD,
        "log_accuracy_scaled":  (np.log1p(accuracy) - LOG_ACC_MEAN) / LOG_ACC_STD,
    }
    ...
    binary = {
        # NOTE: is_outside is intentionally EXCLUDED (label leakage — see notebook)
        "high_accuracy_flag":  int(accuracy < 20),
        "has_seat":            0,          # unknown at inference time
        "accuracy_bin":        int(np.digitize(accuracy, [10, 25, 50, 100])),
        "seat_block_enc":      0,          # unknown at inference time
    ...
    # One-hot encode nearest_amphi (8 columns: nearest_amphi_0 … nearest_amphi_7)
    ohe = {f"nearest_amphi_{i}": int(nearest_idx == i) for i in range(8)}

    row = {**scaled, **binary, **ohe}
...
def classify(lat: float, lon: float, accuracy: float = 25.0) -> dict:
    ...
    1. Run outside_detector first as a gate (avoids the is_outside leakage issue)
    2. If inside, run the main amphi classifier
    3. Fallback to haversine-centroid if models aren't loaded (dev mode)
    """
    dists = {n: haversine(lat, lon, c[0], c[1]) for n, c in CENTROIDS.items()}
    nearest_name = min(dists, key=dists.get)
    nd = dists[nearest_name]
    inv = {k: 1.0 / max(v, 0.5) for k, v in dists.items()}
    s   = sum(inv.values())
    probs = {k: v / s for k, v in inv.items()}   # default soft probabilities

    # ── MODEL PATH ────────────────────────────────────────────────────────────
    main_model    = MODELS.get(ACTIVE_MODEL_KEY)
    outside_model = MODELS.get("outside")
    label_enc     = MODELS.get("label_enc")

    if main_model is not None:
        try:
            features = build_features(lat, lon, accuracy)

            # Step 1 — outside gate
            is_outside_pred = False
            if outside_model is not None:
                is_outside_pred = bool(outside_model.predict(features)[0])

            if is_outside_pred:
                label = "Outside"
            else:
                # Step 2 — amphi classifier
                pred_encoded = main_model.predict(features)[0]
                if label_enc is not None:
                    label = label_enc.inverse_transform([pred_encoded])[0]
                else:
                    # label_enc.pkl not found: map integer back to name directly
                    label = f"Amphi {pred_encoded + 1}" if pred_encoded < 8 else "Outside"

                # Step 3 — use model probabilities if available
                if hasattr(main_model, "predict_proba"):
                    with warnings.catch_warnings():
                        warnings.simplefilter("ignore")
                        raw_probs = main_model.predict_proba(features)[0]
                    classes = (label_enc.classes_ if label_enc
                               else [f"Amphi {i+1}" for i in range(8)] + ["Outside"])
                    probs = {str(c): float(p) for c, p in zip(classes, raw_probs)}

        except Exception as e:
            # If anything goes wrong, fall back to haversine and show a warning
            st.warning(f"Model error — using centroid fallback: {e}")
            on_campus = (36.6876 <= lat <= 36.6892 and 2.8650 <= lon <= 2.8685)
            label = "Outside" if (nd > max(accuracy * 2.0, 50) or not on_campus) else nearest_name

    else:
        # ── FALLBACK (no models loaded — haversine centroid) ─────────────────
        on_campus = (36.6876 <= lat <= 36.6892 and 2.8650 <= lon <= 2.8685)
        label = "Outside" if (nd > max(accuracy * 2.0, 50) or not on_campus) else nearest_name

    return dict(label=label, nearest=nearest_name, dist=nd,
                dists=dists, probs=probs, floor=FLOOR.get(label))
...
.stTabs [data-baseweb="tab"] {
    ...
    font-family: 'Inter', sans-serif !important;
    font-size: 0.83rem !important; font-weight: 500 !important;
    padding: 7px 20px !important; transition: all 0.18s !important;
}
.stTabs [data-baseweb="tab"]:hover {
    color: rgba(255,255,255,0.7) !important;
    background: rgba(255,255,255,0.04) !important;
...
.stat-card::after {
    content: ''; position: absolute;
    top: 0; left: 0; right: 0; height: 1px; border-radius: 14px 14px 0 0;
}
.stat-card.s-purple::after { background: linear-gradient(90deg, #818cf8, transparent); }
.stat-card.s-green::after  { background: linear-gradient(90deg, #34d399, transparent); }
.stat-card.s-cyan::after   { background: linear-gradient(90deg, #67e8f9, transparent); }
...
.result-inside {
    background: linear-gradient(160deg, rgba(4,120,87,0.22), rgba(5,150,105,0.08));
    border: 1px solid rgba(16,185,129,0.35);
    box-shadow: 0 0 50px rgba(16,185,129,0.1), inset 0 1px 0 rgba(255,255,255,0.04);
}
.result-outside {
    background: linear-gradient(160deg, rgba(146,64,14,0.22), rgba(180,83,9,0.08));
    border: 1px solid rgba(245,158,11,0.35);
    box-shadow: 0 0 50px rgba(245,158,11,0.1), inset 0 1px 0 rgba(255,255,255,0.04);
}
.result-label {
    font-family: 'JetBrains Mono', monospace;
    font-size: 0.62rem; letter-spacing: 3px; text-transform: uppercase; margin-bottom: 10px;
}
.result-inside  .result-label { color: rgba(52,211,153,0.7); }
.result-outside .result-label { color: rgba(251,191,36,0.7); }
.result-name {
...
.floor-tag.f1 { background: rgba(99,102,241,0.18); color: #a5b4fc; border: 1px solid rgba(99,102,241,0.3); }
.floor-tag.f2 { background: rgba(168,85,247,0.18); color: #d8b4fe; border: 1px solid rgba(168,85,247,0.3); }
.result-meta { font-size: 0.78rem; color: rgba(255,255,255,0.38); line-height: 2; margin-top: 6px; }
.result-meta b { color: rgba(255,255,255,0.65); font-weight: 500; }
.result-coords {
    font-family: 'JetBrains Mono', monospace; font-size: 0.68rem;
    color: rgba(255,255,255,0.25); margin-top: 8px;
...
.model-badge {
    ...
    padding: 3px 12px; border-radius: 5px; letter-spacing: 0.5px;
...
.conf-bar-bg { flex: 1; height: 5px; border-radius: 999px; background: rgba(255,255,255,0.06); }
.conf-bar-fill { height: 100%; border-radius: 999px; }
.conf-pct {
    font-family: 'JetBrains Mono', monospace; font-size: 0.68rem;
...
.stButton > button {
    ...
    font-weight: 500 !important; padding: 0.45rem 0.8rem !important;
    transition: all 0.15s !important; box-shadow: none !important;
}
.stButton > button:hover {
    background: rgba(129,140,248,0.12) !important; border-color: rgba(129,140,248,0.3) !important;
...
div[data-testid="stButton"]:has(button[kind="primary"]) > button {
    ...
    box-shadow: 0 4px 24px rgba(79,70,229,0.35) !important; letter-spacing: 0.2px !important;
}
...
label[data-testid="stWidgetLabel"] p {
    font-size: 0.78rem !important; color: rgba(255,255,255,0.45) !important;
...
.info-box {
    ...
    border-radius: 10px; padding: 1rem 1.2rem;
    font-size: 0.79rem; color: rgba(255,255,255,0.48); line-height: 1.75; margin-top: 1.4rem;
}
.info-box .info-title {
...
.info-box code {
    font-family: 'JetBrains Mono', monospace; font-size: 0.7rem; color: #67e8f9;
    background: rgba(103,232,249,0.07); padding: 1px 6px; border-radius: 4px; word-break: break-all;
}

.qt-label {
    font-family: 'JetBrains Mono', monospace; font-size: 0.6rem;
    text-transform: uppercase; letter-spacing: 2.5px; color: rgba(129,140,248,0.5);
    margin-top: 1.8rem; margin-bottom: 0.8rem;
}

.log-wrap {
    background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05);
    border-radius: 12px; padding: 0.5rem; max-height: 420px; overflow-y: auto;
}
.log-row {
    display: flex; align-items: center; gap: 12px; padding: 9px 12px; border-radius: 8px;
    transition: background 0.12s; font-family: 'JetBrains Mono', monospace; font-size: 0.71rem;
}
.log-row:hover { background: rgba(255,255,255,0.03); }
.log-ts   { color: rgba(165,180,252,0.5); flex-shrink: 0; width: 52px; }
.log-dot  { width: 7px; height: 7px; border-radius: 50%; flex-shrink: 0; }
...
.about-body {
    font-size: 0.85rem; color: rgba(255,255,255,0.5); line-height: 2;
    background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05);
    border-radius: 14px; padding: 1.6rem;
}
.about-body h4 {
...
[data-testid="stDataFrame"] { border: 1px solid rgba(255,255,255,0.06) !important; border-radius: 12px !important; overflow: hidden; }
[data-testid="stPlotlyChart"] { border-radius: 14px; overflow: hidden; }

.empty-state {
    border: 1px dashed rgba(255,255,255,0.08); border-radius: 16px;
    padding: 3.5rem 2rem; text-align: center; color: rgba(255,255,255,0.2);
}
.empty-state .es-icon { font-size: 1.8rem; margin-bottom: 10px; color: rgba(255,255,255,0.1); font-family: 'Space Grotesk', sans-serif; font-weight: 700; }
.empty-state .es-title { font-family: 'Space Grotesk', sans-serif; font-size: 1rem; color: rgba(255,255,255,0.22); }
.empty-state .es-sub { font-size: 0.78rem; margin-top: 6px; color: rgba(255,255,255,0.15); }
...
.ref-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
.ref-name { color: rgba(255,255,255,0.65); flex: 1; }
.ref-floor { font-family: 'JetBrains Mono', monospace; font-size: 0.66rem; padding: 2px 10px; border-radius: 5px; }
.rf1 { background: rgba(99,102,241,0.15); color: #a5b4fc; border: 1px solid rgba(99,102,241,0.25); }
.rf2 { background: rgba(168,85,247,0.15); color: #d8b4fe; border: 1px solid rgba(168,85,247,0.25); }
</style>
""", unsafe_allow_html=True)
...
n_scans   = len(st.session_state.history)
ip        = get_ip()
active_m  = ACTIVE_MODEL_KEY.upper() if MODELS.get(ACTIVE_MODEL_KEY) else "CENTROID"
...
<div class="app-header">
    ...
    <div class="header-badges">
        ...
        <span class="badge">model: {active_m}</span>
        <span class="badge">8 amphitheatres</span>
        <span class="badge">{n_scans} scans &nbsp;·&nbsp; session {st.session_state.sid}</span>
    </div>
</div>
...
with tab_detect:
    ...
    with col_l:
        st.markdown('<div class="slabel">GPS Coordinates</div>', unsafe_allow_html=True)
        st.markdown("""<div style="font-size:0.78rem;color:rgba(255,255,255,0.38);margin-bottom:8px;line-height:1.6;">
            Paste coordinates in any format — Google Maps, phone browser, or raw numbers.</div>""",
            unsafe_allow_html=True)
        ...
        if _parse_err:
            st.markdown(f'<div style="font-size:0.74rem;color:#f87171;margin-top:2px;font-family:JetBrains Mono,monospace;">{_parse_err}</div>', unsafe_allow_html=True)
        elif raw_input.strip():
            st.markdown(f'<div style="font-size:0.72rem;color:rgba(74,222,128,0.75);margin-top:2px;font-family:JetBrains Mono,monospace;">Parsed: {_lat_default:.7f}, {_lon_default:.7f}</div>', unsafe_allow_html=True)

        c1, c2 = st.columns(2)
        lat = c1.number_input("Latitude",  value=_lat_default, format="%.7f", min_value=36.60, max_value=36.76, key="lat_fine")
        lon = c2.number_input("Longitude", value=_lon_default, format="%.7f", min_value=2.80,  max_value=2.90,  key="lon_fine")
    ...
    with col_r:
        ...
        if r is None:
            ...
            <div class="result-card {cls}">
                ...
                <div class="result-name">{r['label']}</div>
                {floor_html}
                <div class="model-badge">via {active_m}</div>
                <div class="result-meta" style="margin-top:12px;">
                    <b>{r['dist']:.1f} m</b> to centroid
                    &nbsp;&middot;&nbsp; GPS &plusmn;<b>{r.get('accuracy',25):.0f} m</b>
                    &nbsp;&middot;&nbsp; {r.get('source','—')}
                </div>
                <div class="result-coords">
                    {r.get('lat',0):.7f}, {r.get('lon',0):.7f}
            ...
            for name, prob in sorted(r["probs"].items(), key=lambda x: -x[1]):
                ...
                <div class="conf-item">
                    <span class="conf-label">{name}</span>
                    <div class="conf-bar-bg"><div class="conf-bar-fill" style="width:{w};background:{c};opacity:0.85;"></div></div>
                    <span class="conf-pct">{w}</span>
                </div>""", unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
...
with tab_log:
    ...
    if not history:
        st.markdown("""<div class="empty-state" style="margin-top:1rem;">
            <div class="es-icon">--</div>
            <div class="es-title">No scans yet</div>
            <div class="es-sub">Classify a location on the Detect tab to see history here</div>
        </div>""", unsafe_allow_html=True)
        ...
        with col_log:
            st.markdown(f'<div class="slabel">History &middot; {len(history)} entries</div>', unsafe_allow_html=True)
            st.markdown('<div class="log-wrap">', unsafe_allow_html=True)
            for rec in reversed(history[-50:]):
                c   = COLORS.get(rec["label"], "#475569")
                src = {"manual": "kbd", "test": "test", "phone": "phone"}.get(rec.get("source",""), "—")
                st.markdown(f"""
                <div class="log-row">
                    <span class="log-ts">{rec.get('ts','—')}</span>
                    <span class="log-dot" style="background:{c};"></span>
                    <span class="log-name" style="color:{c};">{rec['label']}</span>
                    <span class="log-coord">{rec.get('lat',0):.5f}, {rec.get('lon',0):.5f}</span>
                    <span class="log-src">{src}</span>
                    <span class="log-d">{rec['dist']:.0f} m</span>
                </div>""", unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
        ...
        with col_charts:
            ...
            fig_b = go.Figure(go.Bar(
                x=cnts["label"], y=cnts["count"],
                marker_color=[COLORS.get(l, "#475569") for l in cnts["label"]],
                marker_opacity=0.82, text=cnts["count"], textposition="outside",
                textfont=dict(color="rgba(255,255,255,0.45)", size=11), marker_line_width=0,
            ))
            fig_b.update_layout(
                ...
                yaxis=dict(gridcolor="rgba(255,255,255,0.04)", showline=False, title=""),
                height=240, margin=dict(t=10, b=0, l=0, r=10), bargap=0.35,
            )
            ...
            fig_t.add_trace(go.Scatter(
                x=list(range(1, len(df_h) + 1)), y=df_h["dist"],
                mode="lines", line=dict(color="rgba(129,140,248,0.3)", width=1.5),
                fill="tozeroy", fillcolor="rgba(129,140,248,0.04)", showlegend=False,
            ))
            fig_t.add_trace(go.Scatter(
            ...
            fig_t.update_layout(
                ...
                xaxis=dict(gridcolor="rgba(255,255,255,0.04)", showline=False,
                           title=dict(text="Scan #", font=dict(size=10))),
...
with tab_about:
    ...
    with col_a:
        ...
        <div class="about-body">
            ...
            ENSIA Machine Learning Project &middot; Spring 2025-2026
            <h4>Active model</h4>
            <code>{active_m}</code> &nbsp;&mdash;&nbsp; {model_status}
            <h4>Objective</h4>
            Classify which of ENSIA's 8 amphitheatres a student is sitting in,
            using GPS coordinates read from their phone. 9-class problem: Amphi 1–8 + Outside.
            <h4>Classification pipeline</h4>
            1. <b>outside_detector</b> gates the prediction first (binary: inside/outside)<br>
            2. If inside → <b>{active_m}</b> predicts the exact amphitheatre<br>
            3. <b>label_encoder</b> maps the numeric output back to "Amphi X"<br>
            Fallback to haversine-centroid if models aren't found.
            <h4>Data</h4>
            gps_data_v2.csv — 8,224 clean readings — 9 classes.
            <h4>Limitation</h4>
            GPS cannot distinguish Floor 1 from Floor 2. Rooms on the same building column
            (Amphi 1&5, 2&6, 3&7, 4&8) share the same lat/lon footprint.
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_b:
        ...
        fig_pie = go.Figure(go.Pie(
            ...
            textfont=dict(size=9.5, color="rgba(255,255,255,0.6)"),
            showlegend=False, pull=[0.03] * len(sample_data),
        ))
        fig_pie.update_layout(
            paper_bgcolor="rgba(0,0,0,0)", height=300,
            margin=dict(t=0, b=0, l=0, r=0),
            annotations=[
                dict(text="8,224", x=0.5, y=0.55, font=dict(size=18, color="rgba(255,255,255,0.7)", family="Space Grotesk"), showarrow=False),
                dict(text="readings", x=0.5, y=0.4, font=dict(size=10, color="rgba(255,255,255,0.25)", family="Inter"), showarrow=False),
            ],
        )
